...
one-node path-complete graph and the polynomial PC-CC

    C_{1,1}(x,y) = (y2 - y1) - 0.40001.

The physical trajectories are the same as in the two-node plot because the
switching signal drives the plant dynamics. The one-node graph simply keeps the
abstract graph state fixed at v1 while accepting both modes through self-loops.

Run from repo root:

    python sim/pccc_sim_platoon_polynomial_pccc_1node.py

It writes:

    Figs/platoon_combined_polynomial_pccc_1node.pdf
    Figs/platoon_state_polynomial_pccc_1node.pdf
    Figs/platoon_gap_polynomial_pccc_1node.pdf
    Figs/platoon_closure_polynomial_pccc_1node.pdf
"""
...
# One-node graph: v1 --1--> v1 and v1 --2--> v1.
GRAPH_NODES = [1]
GRAPH_EDGES = [(1, 1, 1), (1, 2, 1)]
...
def switching_random(p: float = 0.5, seed: int = RANDOM_SEED) -> Callable[[int, np.ndarray], int]:
    # Use Python's stdlib RNG to avoid Windows environments that block
    # NumPy's compiled random DLLs.
...
def next_node_one_node(v_curr: int, sigma: int) -> int:
    # Both labels are self-loops at node 1.
    if (v_curr, sigma, 1) not in GRAPH_EDGES:
        raise ValueError(f"No one-node edge for state {v_curr} and mode {sigma}")
    return 1


def simulate_trajectory(x0: np.ndarray, T: int, rule: Callable[[int, np.ndarray], int]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    X = np.zeros((T + 1, 2), dtype=float)
    sigmas = np.zeros(T, dtype=int)
    v_path = np.ones(T + 1, dtype=int)
    X[0] = x0
    v_path[0] = 1
    for t in range(T):
        sigmas[t] = rule(t, X[t])
        X[t + 1] = f_sigma(X[t], int(sigmas[t]))
        v_path[t + 1] = next_node_one_node(int(v_path[t]), int(sigmas[t]))
    return X, sigmas, v_path


def make_trajectories() -> List[Trajectory]:
    x0 = np.array([0.5, 0.8], dtype=float)  # g(x0)=0.3
    rules = [
    ...
    for label, rule, color in rules:
        X, sigmas, v_path = simulate_trajectory(x0, T_HORIZON, rule)
        trajectories.append(Trajectory(X=X, sigmas=sigmas, v_path=v_path, label=label, color=color))
    return trajectories
...
def plot_certificate_values(trajectories: List[Trajectory], ax=None):
    ...
    ax.set_xlabel(r"time step $t$")
    ax.set_ylabel(r"$C_{1,1}(x(t),x(t+1))=g(x(t+1))-0.40001$")
    ax.set_title("One-node polynomial PC-CC values")
    ax.grid(True, alpha=0.5)
    _dedup_legend(ax, loc="lower right")
    return ax
...
def main() -> None:
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(figs_dir, "platoon_state_polynomial_pccc_1node.pdf"), bbox_inches="tight")

    fig, ax = plt.subplots(figsize=(5.5, 4.5))
    plot_gap_trajectories(trajectories, ax=ax)
    fig.tight_layout()
    fig.savefig(os.path.join(figs_dir, "platoon_gap_polynomial_pccc_1node.pdf"), bbox_inches="tight")

    fig, ax = plt.subplots(figsize=(5.5, 4.5))
    plot_certificate_values(trajectories, ax=ax)
    fig.tight_layout()
    fig.savefig(os.path.join(figs_dir, "platoon_closure_polynomial_pccc_1node.pdf"), bbox_inches="tight")
    ...
    fig.suptitle("One-node graph with polynomial PC-CC (SOS template)", y=1.03, fontsize=12)
    fig.tight_layout()
    fig.savefig(os.path.join(figs_dir, "platoon_combined_polynomial_pccc_1node.pdf"), bbox_inches="tight")
    print("Saved one-node figures to", figs_dir)

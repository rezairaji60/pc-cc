...
function main()
    ...
    push!(cases, solve_max_theta("P1+P2+P3 original lambdas", lambda2=0.02, lambda3=0.02))
    push!(cases, solve_max_theta("P1+P2+P3 lambda2=lambda3=0", lambda2=0.0, lambda3=0.0))
    push!(cases, solve_max_theta("P3 only original lambdas", use_p1=false, use_p2=false, use_p3=true, lambda2=0.02, lambda3=0.02))
    push!(cases, solve_max_theta("P3 only lambda2=lambda3=0", use_p1=false, use_p2=false, use_p3=true, lambda2=0.0, lambda3=0.0))
    push!(cases, solve_max_theta("P1+P2+P3 original norm=-0.1", lambda2=0.02, lambda3=0.02, norm_value=-0.1))
    push!(cases, solve_max_theta("P1+P2+P3 original norm=-10", lambda2=0.02, lambda3=0.02, norm_value=-10.0))
    push!(cases, solve_max_theta("P1+P2+P3 no lambdas norm=-10", lambda2=0.0, lambda3=0.0, norm_value=-10.0))

    println("Legacy pwquad max-theta diagnostic on minimal non-vacuous WF grid")
    println("Grid: coarse 7x7, wf 7x41, x0 7x41")
    println("label                                       term            primal              theta/rho/counts")
    println("------------------------------------------------------------------------------------------------")
    for r in cases
        print_row(r)
    end
    mkpath(joinpath("results", "diagnostics"))
    out = joinpath("results", "diagnostics", "legacy_pwquad_max_theta_diagnostic.json")
    open(out, "w") do io
        JSON.print(io, cases, 2)
    end
    println("Saved max-theta diagnostic JSON: ", out)
end

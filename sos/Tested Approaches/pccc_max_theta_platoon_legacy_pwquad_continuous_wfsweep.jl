...
function main()
    ...
    wf_cases = [(7,41,7,41), (9,41,9,41), (11,41,11,41), (15,41,15,41), (21,41,21,41)]
    cases = Dict{String,Any}[]

    for (wf1,wf2,x01,x02) in wf_cases
        for rb in coeff_bounds
            label = @sprintf("orig lambdas coarse15 wf%dx%d x0%dx%d", wf1, wf2, x01, x02)
    ...
    println("Legacy continuous pwquad max-theta WF/rho sweep")
    println("Forward grid: coarse 15x15; WF/X0 grids varied; original lambdas lambda2=lambda3=0.02; continuity=true")
    ...
    out = joinpath("results", "diagnostics", "legacy_pwquad_continuous_max_theta_wfsweep.json")
    open(out, "w") do io
        JSON.print(io, cases, 2)
    end
    println("Saved max-theta WF-sweep diagnostic JSON: ", out)
end
